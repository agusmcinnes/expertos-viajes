...
import { Plus, Edit, Trash2, Save, X, Package, Plane, Bus, Settings, Ship, Hotel, Star, Calendar, DollarSign } from "lucide-react"
import { supabase } from "@/lib/supabase"
import type { TravelPackage, Destination } from "@/lib/supabase"
import { motion } from "framer-motion"
import { SiteConfigManager } from "./site-config-manager"
import { AccommodationManager } from "./accommodation-manager"
...
export function AdminDashboardSimple() {
  ...
  const [accommodationRates, setAccommodationRates] = useState<any[]>([])
      ...
      if (isAdding) {
        ...
        if (error) {
          ...
          if (error.message.includes("transport_type")) {
            ...
            const { data: fallbackData, error: retryError } = await supabase.from("travel_packages").insert([dataWithoutTransport]).select()
            if (retryError) throw retryError
            
            // Guardar alojamientos para el nuevo paquete
            if (fallbackData && fallbackData[0] && accommodations.length > 0) {
              await saveAccommodationsForPackage(fallbackData[0].id)
            }
